"""
Bogotá Urban Green Equity Analysis
==================================
...
Requirements:
- Input GeoJSON: "bogota_with_population_v2.geojson"
  Must contain: geometry, ESTRATO (int), pop_2025 (float), canopy_coverage (float)

Outputs:
- Console: Statistical summaries (ANOVA, Spearman, Moran’s I, etc.)
- Figure: bogota_four_panel_figure.png (4-panel equity visualization)
...
CITY_CRS = "EPSG:3116"          # Bogotá local projected CRS
INPUT_FILE = "bogota_with_population_v2.geojson"
OUTPUT_FIG = "bogota_four_panel_figure.png"
...
if len(groups) > 1:
    kw = kruskal(*groups)
    lv = levene(*groups)
    print(f"\n5️⃣  Kruskal H = {kw.statistic:.3f}, p = {kw.pvalue:.3f}")
    print(f"6️⃣  Levene W = {lv.statistic:.3f}, p = {lv.pvalue:.3f}")
...
if len(data_m) >= 3:
    ...
    print(f"I = {moran.I:.3f}, z = {moran.z_sim:.2f}, p = {moran.p_sim:.3f}")
else:
